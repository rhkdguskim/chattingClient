...
const Content: React.FC<Props> = (props) => {
  const { login, changeMessage, Sociallogin, loginFailuerMsg, loggingIn } =
    props;
  ...
  const handleLoginClick = (event: React.MouseEvent, authProvider: string) => {
    ...
    switch (authProvider) {
      case "kakao":
        popup = window.open(
          `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${KAKAO_REST_API_KEY}&redirect_uri=${KAKAO_REDIRECT_URL}`,
          "kakaoAuth",
      ...
      case "naver":
        popup = window.open(
          `https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=${NAVER_REST_API_KEY}&redirect_uri=${NAVER_REDIRECT_URL}`,
          "naverAuth",
          "width=500,height=500",
        );
      ...
      case "google":
        popup = window.open(
          `https://accounts.google.com/o/oauth2/v2/auth?client_id=${GOOGLE_REST_API_KEY}&redirect_uri=${GOOGLE_REDIRECT_URL}&response_type=code&scope=${GOOGLE_SCOPE}`,
          "googleAuth",
          "width=500,height=500",
        );
  ...
  return (
    <Wrapper>
      ...
      >
        ...
        <KakaoLoginButton onClick={(event) => handleLoginClick(event, "kakao")}>
          {" "}
        </KakaoLoginButton>
        <NaverLoginButton onClick={(event) => handleLoginClick(event, "naver")}>
          {" "}
        </NaverLoginButton>
        <GoogleLoginButton
          onClick={(event) => handleLoginClick(event, "google")}
        >
          {" "}
        </GoogleLoginButton>
